...
let fullNames = [];
runners.forEach(element => fullNames.push(`${element.first_name} ${element.last_name}`));
// console.log(fullNames);

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runners' first names in uppercase because the director BECAME DRUNK WITH POWER. Populate an array called `firstNamesAllCaps`. This array will contain just strings.
let firstNamesAllCaps = [];
runners.map(element => firstNamesAllCaps.push(`${element.first_name.toUpperCase()}`));
...
runners.filter(function(element){ 
  if (element.shirt_size === "L"){
    return runnersLargeSizeShirt.push(`${element.first_name}, ${element.shirt_size}`);
  }
});
...
// Problem 1 //donors over 250
const highestDonors = [] 
runners.filter(function(donors){
  if (donors.donation > 250){
    return highestDonors.push(`${donors.first_name} ${donors.last_name} ${donors.donation}`);
  }
}, 0);
highestDonors.sort((a, b) => a.donation > b.donation)
// console.log(highestDonors);


// Problem 2 runners with names less than 6 characters
const shortNameRunners = [];
runners.filter(function(name){
  if (name.first_name.length < 6){
    shortNameRunners.push(name.first_name.toUpperCase());
  }
})
// console.log(shortNameRunners)
// Problem 3
runnersemail = runners.map(function(runner){
  return runner.email
})
